...
const AttendancePanel: React.FC<Props> = ({ log, config, onUpdate, isFullWidth }) => {
  ...
  return (
    <div className={`app-card border rounded-3xl p-6 backdrop-blur-sm shadow-xl flex flex-col ${isFullWidth ? 'h-full' : ''}`}>
        ...
        {!isEditing && (
          <div className="flex gap-3">
            <button
              ...
              disabled={!!log.timeIn}
              className={`flex-1 py-3 px-4 rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 transition-all ${
                log.timeIn 
